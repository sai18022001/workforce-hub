...
func (_q *UserQuery) QueryAssignments() *ProjectAssignmentQuery {
...
func (_q *UserQuery) QueryTimesheets() *TimesheetQuery {
	query := (&TimesheetClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withTimesheets:  _q.withTimesheets.Clone(),
...
func (_q *UserQuery) WithAssignments(opts ...func(*ProjectAssignmentQuery)) *UserQuery {
	query := (&ProjectAssignmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithTimesheets(opts ...func(*TimesheetQuery)) *UserQuery {
	query := (&TimesheetClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withTimesheets = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	var (
		...
		loadedTypes = [4]bool{
			...
			_q.withTimesheets != nil,
		}
	)
	if _q.withRole != nil || _q.withDepartment != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withAssignments; query != nil {
		if err := _q.loadAssignments(ctx, query, nodes,
			func(n *User) { n.Edges.Assignments = []*ProjectAssignment{} },
			func(n *User, e *ProjectAssignment) { n.Edges.Assignments = append(n.Edges.Assignments, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withTimesheets; query != nil {
		if err := _q.loadTimesheets(ctx, query, nodes,
			func(n *User) { n.Edges.Timesheets = []*Timesheet{} },
			func(n *User, e *Timesheet) { n.Edges.Timesheets = append(n.Edges.Timesheets, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadAssignments(ctx context.Context, query *ProjectAssignmentQuery, nodes []*User, init func(*User), assign func(*User, *ProjectAssignment)) error {
...
func (_q *UserQuery) loadTimesheets(ctx context.Context, query *TimesheetQuery, nodes []*User, init func(*User), assign func(*User, *Timesheet)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Timesheet(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.TimesheetsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_timesheets
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_timesheets" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_timesheets" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
