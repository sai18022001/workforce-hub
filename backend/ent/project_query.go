...
func (_q *ProjectQuery) QueryAssignments() *ProjectAssignmentQuery {
...
func (_q *ProjectQuery) QueryTimesheets() *TimesheetQuery {
...
func (_q *ProjectQuery) WithAssignments(opts ...func(*ProjectAssignmentQuery)) *ProjectQuery {
	query := (&ProjectAssignmentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) WithTimesheets(opts ...func(*TimesheetQuery)) *ProjectQuery {
	query := (&TimesheetClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := _q.withTimesheets; query != nil {
		if err := _q.loadTimesheets(ctx, query, nodes,
			func(n *Project) { n.Edges.Timesheets = []*Timesheet{} },
			func(n *Project, e *Timesheet) { n.Edges.Timesheets = append(n.Edges.Timesheets, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ProjectQuery) loadDepartment(ctx context.Context, query *DepartmentQuery, nodes []*Project, init func(*Project), assign func(*Project, *Department)) error {
	...
	for i := range nodes {
		if nodes[i].department_projects == nil {
			continue
		}
		fk := *nodes[i].department_projects
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "department_projects" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *ProjectQuery) loadAssignments(ctx context.Context, query *ProjectAssignmentQuery, nodes []*Project, init func(*Project), assign func(*Project, *ProjectAssignment)) error {
...
func (_q *ProjectQuery) loadTimesheets(ctx context.Context, query *TimesheetQuery, nodes []*Project, init func(*Project), assign func(*Project, *Timesheet)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Timesheet(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.TimesheetsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.project_timesheets
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_timesheets" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_timesheets" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
