...
func (_q *DepartmentQuery) loadProjects(ctx context.Context, query *ProjectQuery, nodes []*Department, init func(*Department), assign func(*Department, *Project)) error {
	...
	for _, n := range neighbors {
		fk := n.department_projects
		if fk == nil {
			return fmt.Errorf(`foreign-key "department_projects" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "department_projects" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
